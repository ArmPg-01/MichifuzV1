...
handler.before = async (m) => {
  ...
  if (bot.modoia && !m.isGroup && !Prefijo && !m.fromMe && m.text !== '') {
     if (/^.*false|disnable|(turn)?off|0/i.test(m.text)) return;
        ...
        const sytm = await fetch(`https://raw.githubusercontent.com/ArmPg-01/MichifuzV1/master/src/JSON/chatgpt_indicciones.txt`).then(v => v.text());
        const sistema1 = sytm.replace('@name', namedem)
        const sistema2 = 'Tu seras Don Michifuz, un bot de WhatsApp creado por Bruno Sobrino';
        try {
